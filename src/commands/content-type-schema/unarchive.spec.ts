...
describe('content-item-schema unarchive command', () => {
  ...
  describe('handler tests', function () {
    ...
    it('should throw an error when revert log is missing', async () => {
      ...
      expect(logSpy.mock.lastCall).toEqual(['Fatal error - could not read archive log']);
    });
    ...
    it('should throw an error when fails to get content type schema by id', async () => {
      ...
      await expect(handler(argv)).resolves.toBeUndefined();
      expect(logSpy.mock.lastCall).toEqual(['Fatal error: could not find content type schema with ID test-schema-id']);
    });
    ...
    it('should throw an error when fails to get hub by id', async () => {
      ...
      await expect(handler(argv)).resolves.toBeUndefined();
      expect(logSpy.mock.lastCall).toEqual(['Fatal error: could not retrieve content type schemas to unarchive']);
    });
