...
describe('linked-content-repository', () => {
  ...
  it('should update a linked content respository', async () => {
    const yargArgs = {
      $0: 'test',
      _: ['test'],
      json: true
    ...
    const mockUpdate = jest.fn().mockResolvedValue(new LinkedContentRepository(linkedContentRepositoryPayload));
    ...
    expect(mockUpdate).toHaveBeenCalledWith(linkedContentRepositoryPayload);
